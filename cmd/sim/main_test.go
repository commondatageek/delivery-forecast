...
func TestComputeTrajectoryTable_TelescopesToTotal(t *testing.T) {
	...
	// fixed-seed trial, as simulateDaysToComplete guarantees.
	dists := [][]int{
	...
	for pi := range percentiles {
		...
		for g := range dists {
			...
			sum += cell.MarginalDays
		}
		if sum != totals[pi] {
			t.Errorf("percentile %d: sum of marginal days = %d, want total %d", percentiles[pi], sum, totals[pi])
		}
